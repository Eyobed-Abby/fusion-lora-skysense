...
class SkySenseCLIP(nn.Module):
    ...
    This version supports two init modes:

    1) cfg_or_path is a Detectron2 CfgNode:
       - Uses cfg.MODEL.CLIP_CFG_PATH and cfg.MODEL.CLIP_CKPT_PATH
       - Automatically loads pretrained weights from the ckpt

    2) cfg_or_path is a string/path to clip_config.yml:
       - Only builds the model from YAML (no ckpt loaded automatically)
    """

    def __init__(self, cfg_or_path, init_logit_scale: float = np.log(1 / 0.07)):
        super().__init__()

        # Root of the skysense_o repo (contains "configs", "pretrain", "skysense_o", ...)
        repo_root = Path(__file__).resolve().parents[3]

        # Try to detect if we got a Detectron2 cfg or just a path
        try:
        ...
        if CfgNode is not None and isinstance(cfg_or_path, CfgNode):
            # --------- Mode 1: called with Detectron2 cfg ---------
            self.d2_cfg = cfg_or_path.clone()
            print("[SkySenseCLIP] Using Detectron2 cfg.")

            clip_cfg_rel = self.d2_cfg.MODEL.CLIP_CFG_PATH
            clip_cfg_path = (repo_root / clip_cfg_rel).resolve()
            print(f"  CLIP cfg  : {clip_cfg_path}")

            ckpt_rel = getattr(self.d2_cfg.MODEL, "CLIP_CKPT_PATH", None)
            if ckpt_rel is not None:
                ckpt_path = (repo_root / ckpt_rel).resolve()
                print(f"  CLIP ckpt: {ckpt_path}")
        else:
            # --------- Mode 2: old behaviour, called with YAML path ---------
            clip_cfg_path = Path(cfg_or_path).expanduser().resolve()
            print("[SkySenseCLIP] Initialized from YAML path only (no Detectron2 cfg).")
            print(f"  CLIP cfg  : {clip_cfg_path}")
            # ckpt_path stays None -> no auto load
        ...
        # --------- Auto-load pretrained weights if ckpt_path is provided ---------
        if ckpt_path is not None:
            if ckpt_path.is_file():
                self.load_pretrained(str(ckpt_path))
            else:
                print(f"[SkySenseCLIP] WARNING: CKPT path does not exist: {ckpt_path}")
    ...
    def encode_image(self, image, normalize: bool = False, dense=False):
        ...
        if not dense:
            ...
            image_feature = rearrange(
                image_feature,
                "B (H W) C -> B C H W",
                H=np.sqrt(image_feature.shape[1]).astype(int),
            )
            if normalize:
